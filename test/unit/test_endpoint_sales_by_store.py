...
from app.api.routes.sales import get_sales_by_store


@pytest.mark.unit
class TestSalesByStoreEndpoint:
    """Tests para endpoint get_sales_by_store"""
    ...
    async def test_endpoint_calls_service_with_correct_params(self):
        ...
        # Act
        result = await get_sales_by_store(
            key_store='1|023',
            date_start=date(2023, 11, 1),
            date_end=date(2023, 11, 30),
        ...
        # Assert
        mock_service.get_sales_by_store.assert_called_once_with(
            key_store='1|023',
            date_start=date(2023, 11, 1),
            date_end=date(2023, 11, 30)
        )
    ...
    async def test_endpoint_handles_service_value_error(self):
        ...
        # Arrange
        mock_service = Mock()
        mock_service.get_sales_by_store.side_effect = ValueError("Error de validación")
    ...
    async def test_endpoint_handles_service_generic_error(self):
        ...
        # Arrange
        mock_service = Mock()
        mock_service.get_sales_by_store.side_effect = Exception("Error inesperado")
        ...
        assert "Error al consultar ventas por tienda" in exc_info.value.detail
    ...
    async def test_endpoint_returns_all_service_data(self):
        ...
        mock_service.get_sales_by_store.return_value = expected_data
        ...
        # Assert
        assert result['key_store'] == expected_data['key_store']
    ...
    @pytest.mark.asyncio
    async def test_endpoint_accepts_valid_store_key_format(self):
        """El endpoint debe aceptar formato correcto de KeyStore"""
        # Arrange
        mock_service = Mock()
        mock_service.get_sales_by_store.return_value = {
            'key_store': '1|023',
        ...
        valid_keys = ['1|023', '1|001', '2|100', '999|999']
